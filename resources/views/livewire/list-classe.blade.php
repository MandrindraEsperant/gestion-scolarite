    ...
    <div class="p-4 overflow-hidden bg-white shadow-xl sm:rounded-lg">
        ...
        <div class="flex items-center justify-between">
            ...
            <input type="text" class="block mt-1 border-gray-300 rounded" placeholder="Rechercher" wire:model="search"
                wire:poll>
            <a href="{{ route('niveau.classe.create_classe') }}"
                            ...
                            <tbody>
                                @forelse ($classes as $item)
                                    <tr class="border-b-2 border-gray-100">
                                        ...
                                        <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $item->level->libelle }}
                                        </td>
                                        <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $item->level->scolarite }}
                                            ariary</td>
                                        <td class="px-6 py-4 text-sm font-medium text-gray-900">
                                            <a href="{{ route('niveau.classe.edit_classe', $item->id) }}"
                                            ...
                                            {{-- <a href="{{ route('niveau.classe', $item->libelle) }}"
