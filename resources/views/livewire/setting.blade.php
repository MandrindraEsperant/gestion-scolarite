            ...
            <div class="flex flex-wrap justify-center gap-4 p-4">
                @forelse ($schoolYearList as $item)
                    <div class="p-2 sm:w-1/2 md:w-1/3 lg:w-1/4">
                        <div class="w-full h-full min-h-[1px] p-8 bg-white rounded-md shadow-lg cursor-pointer transition duration-500 hover:bg-gray-100 hover:shadow-xl flex flex-col justify-between">
                            <h1 class="px-6 py-4 text-sm font-medium text-gray-900"><span class="text-gray-400">Id : </span>{{ $item->id }}</h1>
                            <h2 class="px-6 py-4 text-sm font-medium text-gray-900"><span class="text-gray-400">Année Scolaire: </span>{{ $item->school_year }}</h2>
                            <p class="px-6 py-4 text-sm font-medium text-gray-900">
                                ...
                                <span class="relative inline-block">
                                    ...
                                    <span class="absolute inset-0 bg-opacity-50 
                                        {{ $item->active == 1 ? 'bg-green-300 blur-sm' : 'bg-red-300 blur-sm' }} 
                                        rounded"></span>
                                </span>
                            </p>
                            <p class="px-6 py-4 text-sm font-medium text-gray-900">
                                <button class="px-2 py-1 text-white {{ $item->active == 1 ? ' bg-red-500 hidden' : ' bg-green-500' }} rounded"
